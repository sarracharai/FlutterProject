...
class _StudentScreenState extends State<StudentScreen> {
  ...
  Future<void> getStudentsByClass(Classe selectedClass) async {
    Response response = await http.get(Uri.parse(
        "http://10.0.2.2:8081/classes/${selectedClass.codClass}/etudiants"));

    if (response.statusCode == 200) {
      dynamic data = jsonDecode(response.body);
      print("JSON data received: $data");

      if (data['_embedded'] != null && data['_embedded']['etudiants'] is List) {
        List<dynamic> etudiantsData = data['_embedded']['etudiants'];

        List<Student> studentsInClass =
            etudiantsData.map((json) => Student.fromJson(json)).toList();
        setState(() {
        ...
        throw Exception(
            "No 'etudiants' data found or data structure is invalid");
      }
    } else {
